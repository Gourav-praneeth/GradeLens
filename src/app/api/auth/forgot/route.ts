...
export async function POST(request: Request) {
  if (!emailEnabled()) {
    return jsonError("Password reset email is not configured on this server.");
  }
  ...
  if (user) {
    try {
      await sendPasswordResetEmail({ id: user.id, email: user.email, name: user.name }, appOrigin(request));
    } catch {
